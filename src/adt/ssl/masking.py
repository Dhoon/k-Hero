...
def _segment_mask_1d(T: int, mask_ratio: float, device: torch.device) -> torch.Tensor:
...
def segment_mask(x: torch.Tensor, mask_ratio: float) -> torch.Tensor:
    ...
    B, T, _ = x.shape
    masks = torch.stack(
        [_segment_mask_1d(T, mask_ratio, x.device) for _ in range(B)]
    )  # (B, T)
    return masks
...
def mixed_mask(x: torch.Tensor, mask_ratio: float, segment_prob: float = 0.7) -> torch.Tensor:
    ...
    for b in range(B):
        if torch.rand(1, device=x.device).item() < segment_prob:
            seg = _segment_mask_1d(T, mask_ratio, x.device)   # (T,)
...
def generate_mask(x: torch.Tensor, epoch: int, ssl_cfg: dict) -> torch.Tensor:
    ...
    if mode == "segment":
        return segment_mask(x, mask_ratio)
    ...
    if mode == "mixed":
        ...
        return mixed_mask(x, mask_ratio, segment_prob=seg_prob)
